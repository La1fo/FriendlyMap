...
async def show_tickets(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for ticket in tickets:
        kb = InlineKeyboardMarkup(
            [[InlineKeyboardButton("Открыть чат", callback_data=f"ticket_open_{ticket.id}")]]
        )
        await update.message.reply_text(
            f"🎫 Тикет #{ticket.id} (пользователь {ticket.user_id})",
            reply_markup=kb,
        )
    return TICKETS_MENU
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("ticket_open_"):
        ...
        with get_db_context() as db:
            messages = TicketService.list_messages(db, ticket_id)
        history = ""
        if messages:
            history = "\n".join(
        ...
        await query.edit_message_text(
            f"Открыт чат тикета #{ticket_id}.\n"
            f"{history if history else 'История пуста.'}\n\n"
            "Напишите сообщение. Для закрытия используйте кнопку ниже.",
    ...
    if data.startswith("ticket_close_"):
        ticket_id = int(data.split("_")[-1])
        return await close_ticket(update, context, ticket_id)
...
async def ticket_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not ticket_id:
        await update.message.reply_text("Тикет не выбран.", reply_markup=MENU_KEYBOARD)
        return MENU
    with get_db_context() as db:
        if not _check_access(db, update.effective_user.id):
            await update.message.reply_text("⛔ Доступ запрещен.")
        ...
        if not ticket or ticket.status != "open":
            await update.message.reply_text("Тикет закрыт.", reply_markup=MENU_KEYBOARD)
            return MENU
        TicketService.add_message(db, ticket_id, "moderator", update.effective_user.id, update.message.text)
...
async def close_ticket(update: Update, context: ContextTypes.DEFAULT_TYPE, ticket_id: int):
    with get_db_context() as db:
        ...
        ticket = db.get(Ticket, ticket_id)
        if not ticket or ticket.status != "open":
            await update.callback_query.edit_message_text("Тикет уже закрыт.")
            return MENU
        TicketService.close_ticket(db, ticket)
    context.user_data.pop("ticket_id", None)
    await update.callback_query.edit_message_text("✅ Тикет закрыт.")
    return MENU
...
moderation_panel_handler = ConversationHandler(
    ...
    states={
        ...
        TICKETS_MENU: [
            CallbackQueryHandler(handle_callback, pattern=r"^ticket_open_\d+$"),
            MessageHandler(filters.TEXT & ~filters.COMMAND, handle_menu),
        ],
        TICKET_CHAT: [
            MessageHandler(filters.TEXT & ~filters.COMMAND, ticket_message),
            CallbackQueryHandler(handle_callback, pattern=r"^ticket_close_\d+$"),
        ],
    },
