...
def get_rank_progress(total_gp: int, leaderboard_position: int | None = None) -> tuple[int, int]:
    payload = shared_get_rank_progress(total_gp, leaderboard_position=leaderboard_position)
    return int(payload["rank_level"]) - 1, int(payload["gp_in_rank"])


def get_rank_points(total_gp: int, leaderboard_position: int | None = None) -> int:
    payload = shared_get_rank_progress(total_gp, leaderboard_position=leaderboard_position)
